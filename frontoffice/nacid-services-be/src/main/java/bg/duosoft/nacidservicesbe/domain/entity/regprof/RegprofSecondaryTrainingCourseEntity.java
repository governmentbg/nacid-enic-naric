 ...
 */
@Entity
@Table(name = "regprof_secondary_training_course", schema = "services")
@Getter
@Setter
@Cacheable(false)
public class RegprofSecondaryTrainingCourseEntity implements Serializable {
    ...
    @Column(name = "professional_qualification_id")
    ...
    @JoinColumnsOrFormulas({
            @JoinColumnOrFormula(formula=@JoinFormula(value="'QUALIFICATION_RANK'", referencedColumnName="domain")),
            @JoinColumnOrFormula(column = @JoinColumn(name = "qualification_rank", referencedColumnName="code"))
    })
    private ReferenceDataEntity qualificationRank;
}
