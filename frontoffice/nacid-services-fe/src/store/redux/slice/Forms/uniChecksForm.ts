...
import { initialUniChecksApplication } from "../../../../init/uniChecksInitialValues";
...
export const fetchUniChecksForm = createAsyncThunk("uniChecksForm/fetchStatus", async (id: number | string) => {
  const response = await getApplicationById(baseEndpointPaths.uniChecks, id)();
  return response;
});

const uniChecksFormSlice = createSlice({
  name: "UniChecksStoreSlice",
  initialState: initialUniChecksApplication,
  reducers: {
    ...
    resetUniChecksRequest: () => initialUniChecksApplication,
    presetUniCheckServiceKind: (state, action) => {
      state.educationDetails[action.payload] = true;
    },
  ...
  extraReducers: (builder) => {
    builder.addCase(fetchUniChecksForm.fulfilled, (state, action) => {
      state.dataStateStatus = THUNK_STATUS.FULFILLED;

      const updatedValues = { ...initialUniChecksApplication };
    ...
    builder.addCase(fetchUniChecksForm.pending, (state, action) => {
      state.dataStateStatus = THUNK_STATUS.PENDING;
    });
    builder.addCase(fetchUniChecksForm.rejected, (state, action) => {
...
export const {
  ...
  resetUniChecksRequest,
  presetUniCheckServiceKind,
  setStepApplicantEdited,
  setStepEducationEdited,
  setStepDocumentsEdited,
} = uniChecksFormSlice.actions;
export default uniChecksFormSlice.reducer;
