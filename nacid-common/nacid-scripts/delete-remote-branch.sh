...
echo -e "${GREEN}This script will delete branch with the given name directly in gitlab for NACID project.\nIf branch with the same name does not exist in any of the repositories it will just skip the repository.${ENDCOLOR}\n"
...
if [ "$branchName" = "master" ]; then
  echo -e "${RED}master branch cannot be deleted!${ENDCOLOR}"
  exit 1
fi
...
echo -e "\nBranch with name ${GREEN_BOLD}$branchName${ENDCOLOR} will be deleted in (https://gitss.sirma.com/duosoft/nacid) ${GREEN}$repositoriesString${ENDCOLOR}\n"
...
do
  ...
  url="https://gitss.sirma.com/api/v4/projects/$( url_encode $projectName )/repository/branches/$( url_encode $branchName )"
  ...
  if [ "$message" = "404 Branch Not Found" ]; then
    status="${RED}Branch not found${ENDCOLOR}"
  elif [ "$message" = "401 Unauthorized" ]; then
    echo -e "${RED}Invalid git lab token. Access denied.${ENDCOLOR}"
    exit 1
  elif [ "$message" = "403 Forbidden" ]; then
    echo -e "${RED}Operation forbidden. Check access token role and scopes!${ENDCOLOR}"
    exit 1
  elif [ "$message" != "" ]; then
	status="${RED}$message${ENDCOLOR}"
  else 
	status="${GREEN}SUCCESSFUL${ENDCOLOR}"
  fi
  
  result=`printf '%s%*.*s%s %s' "${GREEN}$i${ENDCOLOR}" 0 $((50 - ${#i} )) "$pad" " $status" `

  echo -e "Deleting branch in $result"
done
