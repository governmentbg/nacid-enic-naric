...
const SARAppsListTable = ({ total, records, blockTable, onPageOrOrderChange, group, applicationGroup }) => {
              ...
              {records.map((row, index) => (
                <TableRow hover key={row.id}>
                  ...
                  <TableCell style={{ width: "50px" }}>
                    ...
                    {row?.serviceTypeId === ReferenceDataCode.SERVICE_TYPE_EXPRESS && (
                      <ExpressServiceTypeLabel marginBottom={3} />
                    )}
                  </TableCell>
                  <>
                  ...
                  <OptionTableCell>
                    <TableButton type={"view"} to={`/sar-applications/view/${row.id}`} />
                    <SecurityGuard checkForRoles={[SecurityRole.RudiApplicationEdit]}>
                      <TableButton type={"edit"} to={`/sar-applications/edit/${row.id}`} />
                    </SecurityGuard>
                  </OptionTableCell>
                </TableRow>
              ))}
...
export default SARAppsListTable;
