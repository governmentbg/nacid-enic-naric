...
import { fetchInquiryForm } from "../../../../store/redux/slice/Forms/inquiryForm";
...
const InquiryEdit = () => {
  ...
  return (
    <AppPageContentWrapper>
      <PageContentBox>
        <Box sx={{ width: "100%" }}>
          {inquiryForm.dataStateStatus === THUNK_STATUS.FULFILLED ? (
            <ServiceStepper serviceSteps={inquiryForm.steps} applicationSubtype={ApplicationSubtype.INQUIRY} />
          ) : null}
          {inquiryForm.dataStateStatus === THUNK_STATUS.REJECTED ? (
            <Alert severity={"error"}>{t("m.generic.error.service.fail")}</Alert>
          ) : null}
          {inquiryForm.dataStateStatus === THUNK_STATUS.PENDING ? <CircularTextLoader /> : null}
        </Box>
      </PageContentBox>
    </AppPageContentWrapper>
  );
