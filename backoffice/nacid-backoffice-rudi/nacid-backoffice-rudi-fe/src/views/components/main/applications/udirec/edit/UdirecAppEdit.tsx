...
function a11yProps(key) {
  return {
    id: `diploma-rec-modification-section-${key}`,
    "aria-controls": `diploma-rec-modification-section-tabpanel-${key}`,
  };
}
...
const tabsWithEFiling = [
  {
    key: 0,
    title: "t.efilledData",
    component: <EFilledData appType={AppType.UDIREC_APPLICATION} />,
  },
  {
    key: 1,
  ...
  },
  {
    ...
    component: <AttachmentsData appType={AppType.UDIREC_APPLICATION} />,
  },
  {
    key: 4,
    title: "t.statusData",
    component: <StatusData appType={AppType.UDIREC_APPLICATION} />,
  },
  {
    key: 5,
    title: "t.closingData",
    component: <ClosingData appType={AppType.UDIREC_APPLICATION} />,
  },
];
...
const UdirecAppEdit = ({ id }: { id: string }) => {
  const { t } = useTranslation();
  const { activeTab } = useAppSelector((state) => {
    return state.ComponentsControl.udirecApplicationsControl.editPageTab;
  });
  const executeChangeTabFn = useRudiFormDirtyStateProcessor();

  const handleChange = (event, newValue) => {
    executeChangeTabFn({
      proceedFnKey: FormDirtyStateProcessorFn.handleTabChangeUdirec.name,
          ...
          <Box>
            ...
            {tabs.map((tab) => (
              <div
                ...
                id={`diploma-rec-modification-section-tabpanel-${tab.key}`}
                aria-labelledby={`diploma-rec-modification-section-tab-${tab.key}`}
              >
