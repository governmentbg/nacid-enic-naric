...
select a.id, a.entry_num, a.entry_date, a.status_code, a.docflow_status_code, a.applicant_name, a.application_country_name, a.recognized_profession_name,
       case when status_code = 'WEAK' THEN coalesce(nullif(invalidated_cert_numbers,'')||', ','')||coalesce(denial_decision_docflow_number,'')
...
where a.status_code in ('CE', 'CQ', 'CQE') or a.status_code = 'WEAK' or a.status_code = 'DEN'