...
@Mapper(componentModel = "spring", uses = {
        CountryMapper.class,
        BibliographicReferenceDetailsMapper.class,
...
public abstract class BibliographicReferenceApplicationMapper extends BaseApplicationObjectMapper<BibliographicReferenceFullEntity, BiblioReferenceApplicationDTO> {

    @Autowired
    private BibliographicReferenceDetailsMapper bibliographicReferenceDetailsMapper;

    @BeanMapping(ignoreByDefault = true)
    @Mapping(target = "id", source = "id")
    @Mapping(target = "application", source = "applicantDetails")
    public abstract BibliographicReferenceFullEntity toEntity(BiblioReferenceApplicationDTO biblioReferenceApplicationDTO);

    @InheritInverseConfiguration(name = "toEntity")
    @Mapping(target = "bibliographicReferenceDetails", source = ".")
    ...
    public abstract BiblioReferenceApplicationDTO toDto(BibliographicReferenceFullEntity bibliographicReferenceFullEntity);

    @AfterMapping
    public void afterToEntity(@MappingTarget BibliographicReferenceFullEntity target, BiblioReferenceApplicationDTO source){
        afterToApplicationEntity(target, source);
        if(source != null && source.getBibliographicReferenceDetails() != null){
            BibliographicReferenceFullEntity mappedEntity = bibliographicReferenceDetailsMapper.toEntity(source.getBibliographicReferenceDetails());
            BibliographicReferenceDetailsMapper.copyDetailsToApplication(target, mappedEntity);
        }
    }
