...
const FoUdirecAppsListTable = ({ total, records, blockTable, onPageOrOrderChange, group }) => {
          ...
          <TableBody>
            {records.map((row, index) => (
              <TableRow hover key={row.id}>
                ...
                <TableCell>
                  {row.entryNumber && row.entryDate && (
                    <Typography style={{ whiteSpace: "nowrap" }}>
                      <EntryNumberLabel
                        row={row}
                        url={generateUrl(AppType.UDIREC_APPLICATION, row.boApnId, "edit")}
                ...
                <OptionTableCell>
                  <TableButton type={"view"} to={`/udirec-e-apps/${row.id}/view`} />
                  <SecurityGuard checkForRoles={[SecurityRole.FoServicesAccept]}>
                    <AcceptTableButton appType={AppType.UDIREC_APPLICATION} record={row} />
                  </SecurityGuard>
...
export default FoUdirecAppsListTable;
