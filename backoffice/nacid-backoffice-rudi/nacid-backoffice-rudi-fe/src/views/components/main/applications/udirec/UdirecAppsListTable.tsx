...
const UdirecAppsListTable = ({ total, records, blockTable, onPageOrOrderChange, group, applicationGroup }) => {
            ...
            <TableBody>
              {records.map((row, index) => (
                <TableRow hover key={row.id}>
                  ...
                  <OptionTableCell>
                    <TableButton type={"view"} to={`/udirec-applications/view/${row.id}`} />
                    <SecurityGuard checkForRoles={[SecurityRole.RudiApplicationEdit]}>
                      <TableButton type={"edit"} to={`/udirec-applications/edit/${row.id}`} />
                    </SecurityGuard>
                  </OptionTableCell>
                </TableRow>
              ))}
...
export default UdirecAppsListTable;
