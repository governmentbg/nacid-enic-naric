...
import FoUdirecAppsList from "./FoUdirecAppsList";
...
const FoUdirecApps = () => {
  ...
  const group = SEARCH_FILTERS_GROUP.FO_UDIREC_APPLICATIONS;

  const { methods, handleSubmit } = useReactHookForm<FoApplicationFilterDetails>({
    defaultValues: foUdirecFilterInitialValues,
  ...
  const control = useSearchTableControl({
    group,
    methods,
    initialValues: foUdirecFilterInitialValues,
