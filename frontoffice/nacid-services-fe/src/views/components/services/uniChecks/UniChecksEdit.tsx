...
import { fetchUniChecksForm } from "../../../../store/redux/slice/Forms/uniChecksForm";
...
const UniChecksEdit = () => {
  ...
  useEffect(() => {
    dispatch(fetchUniChecksForm(id));
  ...
  return (
    <AppPageContentWrapper>
      <PageContentBox>
        <Box sx={{ width: "100%" }}>
          {uniChecksForm.dataStateStatus === THUNK_STATUS.FULFILLED ? (
            <ServiceStepper serviceSteps={uniChecksForm.steps} applicationSubtype={ApplicationSubtype.UNI_CHECKS} />
          ) : null}
          {uniChecksForm.dataStateStatus === THUNK_STATUS.REJECTED ? (
            <Alert severity={"error"}>{t("m.generic.error.service.fail")}</Alert>
          ) : null}
          {uniChecksForm.dataStateStatus === THUNK_STATUS.PENDING ? <CircularTextLoader /> : null}
        </Box>
      </PageContentBox>
    </AppPageContentWrapper>
  );
};
export default UniChecksEdit;
